...
- **Integration Tests**: Added comprehensive, end-to-end testing pipelines for the entire Nexus download flow, guaranteeing stability for NXM link parsing, Collection fetching, and auto-updating logic.

### Fixed
- **Mod Extraction Bug**: Fixed a bug where mods extracted to symlinked temporary directories (like `/var` vs `/private/var` on macOS) would fail to move into the active `Mods` folder.
