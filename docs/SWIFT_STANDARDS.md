...
- **A green local `xcodebuild` is not proof a Swift 6 concurrency change is portable.** A dev machine can carry a newer, more lenient Xcode/Swift toolchain than CI's pinned version, and it will silently accept code CI's `xcodebuild` rejects — this bit twice during the Xcode migration (`XCTestCase`'s synchronous `setUpWithError()`/`tearDownWithError()` touching `@MainActor` state; a non-isolated `static func` calling `XCUIApplication`'s `@MainActor`-isolated APIs). For any concurrency-sensitive change, treat the CI run as the real verification, not the local build.
...
**Mutating Nexus calls get their own, separate gate — never folded into `STARHUB_SKIP_LIVE_TESTS`.** `endorseMod` posts a real, externally-visible endorsement to whichever account owns the configured API key — running it isn't idempotent or "reset by re-running" like every other `Integration/` call. `NexusEndorseIntegrationTests` additionally requires `STARHUB_RUN_MUTATING_NEXUS_TESTS=1` (`LiveTestGate.isMutatingTestsSkipped`), which defaults to *not* running even when `Integration` is otherwise selected and a real key is configured — a developer enabling live tests for routine coverage should never, silently, also mutate a real account. Don't fold a future account-mutating call into the plain `isSkipped` gate; give it the same treatment.
...
`Integration/` is a different kind of test on purpose (verifying the real HTTP/parsing contract, not a DI consumer), not a lapse in stub discipline. Every file in it starts with `try XCTSkipIf(LiveTestGate.isSkipped, ...)` (`Tests/Integration/LiveTestGate.swift`) — set `STARHUB_SKIP_LIVE_TESTS=1` to skip all of them at once; CI does this by default. Every test also separately no-ops (`try XCTSkipIf(LiveTestGate.nexusApiKey.isEmpty, ...)`) if no real Nexus API key is configured locally — entering one once in the app's own Settings screen (`SettingsView`'s `@AppStorage`) is all that's needed, no separate test configuration. If a test needs a live external call, it goes in `Integration/` with that gate — never bare in `Services/`/`Features/` with no way to skip it. Beyond the three original client-level tests (`NexusCollectionTests`, `NXMDownloadIntegrationTests`, `ModUpdateTests`), the folder also covers the *wiring* a real click actually runs — `ModInfoIntegrationTests` (raw `modInfo`), `ModUpdateWiringIntegrationTests` (through `ModsStore`/`NexusDownloader`, not the client directly), `NXMOpenURLIntegrationTests` (through `AppCoordinator.handleOpenURL`, the real `nxm://` entry point), `SyncTagIntegrationTests` (through `ModsStore.syncTagFromNexus` for one mod), and `NexusEndorseIntegrationTests` (account-mutating, gated separately — see above). All of these standardize on Nexus mod 1536 ("Mail Framework Mod") as the shared, small, low-traffic test target.

**`UserDefaults(suiteName:)` returns `nil` when the suite name matches the *calling* process's own bundle ID.** `StarHubTHTests` hosts inside the `StarHubTH.app` bundle (`TEST_HOST`), so `UserDefaults(suiteName: "com.appleboiy.StarHubTH")` called from a test — same bundle ID as the calling process — silently returned `nil`, and `?? ""` swallowed it into "no API key configured." Every `Integration/` test reading the key this way had been **silently self-skipping on every single run**, undetected because a skip looks identical to "ran fine, nothing to assert." `UserDefaults.standard` reads the same domain correctly in this hosted-test setup and is what `LiveTestGate.nexusApiKey` now uses. This is a one-directional lesson, not a blanket rule: `StarHubTHUITests` runs as its own separate XCUITest-Runner process (a genuinely different bundle ID from the app it launches), so `suiteName:` there — `LiveUITestGate.nexusApiKey` — is correct and `.standard` would be wrong. Know which process you're actually reading `UserDefaults` from before picking between them.
...
**`StarHubTHUITests` round-trip coverage.** Beyond `SmokeUITests`, four suites drive the real app through a full user-visible pipeline, not just static content on launch: `ModInstallRoundTripUITests` (install → appears in the list), `ModConfigEditRoundTripUITests` (edit a field → Save → verify `config.json` on disk), `ModToggleRoundTripUITests` (click the row toggle → folder actually moves between `Mods/`/`Mods_disabled/` on disk), `LogObserverRoundTripUITests` (append to the live-tailed SMAPI log → the line appears with no restart). None of the four touch a real Stardew Valley install or the real per-user SMAPI log — `StarHubTH/App/UITestFixture.swift` is a composition-root seam, active only when `STARHUB_UITEST_GAME_DIR` is present in the process environment (set by `AppLauncher.launch(environment:)`), that points `AppEnvironment`/`ModsStore`/`LogStore` at an isolated temp `gameDir` and (via `STARHUB_UITEST_FIXTURE_MOD_PATH`) substitutes a fixed URL for `NSOpenPanel`, which XCUITest cannot drive. `StarHubTHUITests/Sources/UITestEnvironment.swift` builds the actual fixture files per test. All three env vars are inert — unset — for every real user launch.

**`TestPlans/UILive.xctestplan` — real API key + real GUI, at once.** `ModDetailLiveDataUITests`/`SyncTagLiveDataUITests` opt a fourth env var, `STARHUB_UITEST_USE_REAL_API_KEY=1`, into the same seam: `UITestFixture.preferenceStoring` then seeds the isolated `.uitest` preferences suite with the real `nexusApiKey` read from `UserDefaults.standard` (the launched app process's own domain — correct here for the same reason it's correct in `LiveTestGate`, see below). `UITestEnvironment.makeWorkspace(nexusModId:)` embeds `"nexus:1536"` in the fixture manifest's `UpdateKeys` so `mod.nexusUrl` resolves to the same canonical mod (`Mail Framework Mod`) every `Integration/` test already targets. Each test self-skips via `XCTSkipIf(LiveUITestGate.nexusApiKey.isEmpty, ...)` if no key is configured — same discipline as `Integration/`, different gate because it runs in a different process (see the `UserDefaults(suiteName:)` lesson below).

Two lessons from building this, both worth knowing before extending it further:

- **A child view inside `ModListRow` cannot carry its own `accessibilityIdentifier`.** The row's own `mod-row-<folder>` identifier (load-bearing — `SmokeUITests` already depends on it) takes over the whole row's accessibility subtree on macOS; a descendant `.accessibilityIdentifier` is silently clobbered, not merged. Locate a specific child by combining the row's identifier with something else that's still unique to that child instead: `ModConfigEditRoundTripUITests` uses the gear button's own accessibility label (`identifier == 'mod-row-FixtureMod' AND label == 'gearshape'` — the SF Symbol name, locale-independent); `ModToggleRoundTripUITests` uses element type (`identifier == 'mod-row-FixtureMod' AND elementType == checkBox`, since the toggle is the row's only `checkBox`-typed descendant and its own label is empty from `.labelsHidden()`). Don't add a doomed identifier inside a row; query this way instead. **This bit twice**: the info button's own `mod-row-info-button-<folder>` identifier was added later (Tier C) and looked fine because it only ever got a compile check, never actually run — `ScreenshotCaptureTool`'s first real capture against a 100+-mod real library was the first thing to actually click it, and it failed every time. Removed from `ModListRow.swift`; `ModDetailLiveDataUITests`/`SyncTagLiveDataUITests`/`scripts/screenshot_manifest.json` all now use the row-identifier-plus-label pattern instead. If a new UI test only ever gets `build-for-testing`-checked and never actually executed against a real UI once, this class of bug hides indefinitely — worth actually running new UI test code at least once, not just confirming it compiles.
- **A closure lexically nested in a `@MainActor` type defaults to `@MainActor`-isolated even when it's actually invoked from a raw GCD callback thread that is never the main actor.** `LogStore.tailSmapiLog(from:)`'s `DispatchSourceFileSystemObject` event handler hit exactly this: the compiler assumed main-actor isolation for a closure GCD calls from `DispatchQueue.global`, and calling `continuation.yield` from the wrong actual thread crashed with a hard `dispatch_assert_queue` runtime failure — reachable any time a line lands in the SMAPI log while the Logs tab is open. Fixed by marking `tailSmapiLog`/`smapiLogPath` `nonisolated`, so the declared isolation matches the real runtime behavior. Neither this crash nor the identifier-clobbering above was caught by 79 unit tests or the two original `SmokeUITests` — only a live round-trip UI test reached them. (Found on local Xcode 26.6; not yet independently confirmed against CI's pinned Xcode 16.2 — watch the first real CI run.)
