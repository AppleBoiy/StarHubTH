...
**Confirmed out of scope, verified by grep**: `docs/DOMAIN_CONTEXT.md` (no build-system mentions) and `docs/QOC_PLAN.md` (its own nearly-finished tracking file — not folded into this migration). `CHANGELOG.md`, `release.py`'s explanatory comment, and `scripts/generate_localizable_strings.py`'s docstring keep their `build_app.py` mentions deliberately — historical narrative, not live claims about current mechanics (same standard `docs/QOC_PLAN.md` F.3 already established).
