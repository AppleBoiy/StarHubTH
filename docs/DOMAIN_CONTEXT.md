...
- Published on Nexus Mods itself as a tool page (`assets/nexus_description.txt` is the Thai-language listing copy, `assets/nexus_description_en.txt` is the English counterpart — since the author also ships a companion [Thai translation pack](https://github.com/AppleBoiy/stardew-thai-translations)). Both stay fully hand-maintained, pasted directly into the Nexus web listing — not read by any build script. `assets/nexus_changelog.txt` is different: `scripts/generate_nexus_changelog.py` drafts each new entry mechanically from `CHANGELOG.md` (skipping internal-only versions), but it's still reviewed, Thai-translated by hand, and pasted manually — never auto-written or auto-posted. Also, `release.yml` auto-uploads every tagged release's build to Nexus's existing file slot (`Nexus-Mods/upload-action`, `docs/RELEASING.md`) — that part needs no manual upload anymore. The app is bilingual (en/th) because its primary audience is the Thai Stardew Valley modding community.
