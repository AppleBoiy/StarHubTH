...
**Current state (verified 2026-07-25): Phase 0–2 done except 0.2/0.3 (partial characterization coverage); Phase 3 done in full; Phase 4.1 done.** Tree already has the `App/Features/DesignSystem/Localization/Support` layout. All 9 I/O-boundary protocols exist, `NexusAPIService` is renamed `LiveNexusAPIClient`, every protocol has a `Stub*` in `Tests/Stubs/`, `App/DependencyContainer.swift` exists as the composition root (not yet handed to anything), and `StarHubTHViewModel` goes through `FilePicking`/`PreferenceStoring` at the 3.4/3.5-named call sites. `LocalizationStore` is the first store extracted from the ViewModel (Localization/LocalizationStore.swift), with the ViewModel forwarding to it. `.shared` singletons are still constructed directly everywhere else — DependencyContainer isn't consumed by anything yet. Next open step is **4.2 `LogStore`**. Note on store extraction generally: per-store recipe says `@MainActor`, but a store is only safe to annotate that way once its caller (still the nonisolated `StarHubTHViewModel` for now) can call into it without a hard actor-isolation error — verify with a build before assuming the recipe's `@MainActor` applies to each new store; it didn't for 4.1. Note: this doc was edited concurrently by multiple sessions working the same plan — always re-verify against `git log` before trusting these checkboxes blindly.
