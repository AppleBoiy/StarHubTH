...
**Current state (verified 2026-07-25): Phase 0–3 done in full (except 0.2/0.3's partial characterization coverage); Phase 4 fully done, including 4.9.** `StarHubTHViewModel.swift` no longer exists. All 8 stores (`LocalizationStore`, `LogStore`, `ThaiHubStore`, `ProfilesStore`, `ModPacksStore`, `SavesStore`, `ModsStore`, `AppEnvironment`) plus two new pieces (`AlertStore`, `AppCoordinator`) are injected via `.environmentObject` from `MainView`, which is now the actual composition root and finally uses `App/DependencyContainer.swift` for real. Every view holds only the specific stores/coordinator it needs — Definition of Done's "`StarHubTHViewModel` no longer exists" is met. Test suite has grown to 196 assertions.

Lessons from 4.1–4.8 that held up through 4.9, plus what changed:
1. **`@MainActor` timing.** The per-store recipe says `@MainActor final class`, but a store is only safe to annotate that way once its caller can call into it without a hard actor-isolation error. Still true post-4.9 — none of the 8 stores are `@MainActor` yet; that's still Phase 5.3's job.
2. **Reactivity: check whether state mutates through an exposed setter or from inside the store's own methods.** This distinction is *why* 4.9's view→singleton bypasses (`SavesView` calling `SaveNotesStore.shared` and manually poking `vm.objectWillChange.send()`) were breakable at all — the manual poke was covering for a store method (`SavesStore.setNote`) that already self-published; the view just wasn't calling it. Once views observe stores directly via `@EnvironmentObject`, this whole class of manual-forwarding workaround disappears — SwiftUI reacts to the store's own `@Published` changes with no relay needed.
3. **Cross-store dependencies before every store exists.** True throughout 4.1–4.8. By 4.9, every store existed, so the ~40 cross-store methods that used to take `gameDir`/`showModal`/`log`/`refresh` as parameters/closures on the ViewModel didn't need redesigning — `AppCoordinator` relocated them near-verbatim, just reading from real store references instead of ViewModel properties.
4. **Definite-initialization order.** Applied identically to `AppCoordinator`'s construction inside `MainView.init()` — `AppCoordinator` must be built after all 8 stores exist, in the same way `StarHubTHViewModel.init()` had to build stores before wiring `objectWillChange` subscriptions.
5. **Testing a store whose method wraps `DispatchQueue.main.async` with no completion callback.** Unchanged, still the pattern in `Tests/main.swift`.
6. **(New in 4.9) A custom `init(vm:...)` that reads `vm.someStore.someMethod()` synchronously to seed a default `@State` value breaks once that store becomes `@EnvironmentObject`** — environment objects aren't available until the view is placed in the hierarchy, unlike an explicit init parameter. Fix: default the `@State` empty and set it in `.onAppear` instead. Hit repeatedly — `SaveEditorView`, `BackupRow`, `DuplicateSaveSheet`, `BranchBackupSheet`, `HomeView`'s `smapiInstaller`.
7. **(New in 4.9) Dropping `vm:` arguments across a large, already-big SwiftUI `if`/`else if` expression can push the type-checker past its time budget** ("unable to type-check this expression in reasonable time"), even though the change is semantically trivial. Not a correctness bug — a compiler complexity cliff. Fix: extract the branch into its own `@ViewBuilder` computed property (or a plain computed property for a smaller sub-expression), which gives the type-checker a smaller, independent unit to solve. Hit in `MainView`'s tab-routing `Group` and its account-badge active-profile lookup.
8. **(New in 4.9) A naive `vm.mods` → `store.mods` string substitution also matches inside `vm.modsStore`** (produces `modsStore.modsStore`, since `vm.mods` is a substring prefix of `vm.modsStore`) — caught immediately by the next build (`modsStore.modsStore` doesn't type-check), so not dangerous, but it did cost a build cycle. Worth ordering substitution rules longest-prefix-first, or just re-reading the diff before building.
