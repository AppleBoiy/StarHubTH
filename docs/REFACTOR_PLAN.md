...
- [ ] 4.9 Delete `StarHubTHViewModel` — **not started; this is a different kind of step than 4.1–4.8.** Every store extraction so far deliberately used a forwarding-shim on the ViewModel specifically so no view file needed to change — that's what kept each step build-verified and low-risk. Actually deleting the ViewModel means the opposite: every view (~20 files) that does `@ObservedObject var vm: StarHubTHViewModel` has to change to hold the specific `@EnvironmentObject` stores it needs instead, `DependencyContainer` has to actually be registered and injected via `.environmentObject(...)` at `MainView`, `alertMessage`/`showAlert` need a home (view `@State` per the per-store recipe, or a dedicated store), and the cross-store orchestrators (`refresh()`, `handleOpenURL`, `selectGameDir()`) need to land somewhere. The `L(_:)` → `subscript(_:)` rename the plan flags as separate work for 4.1 (408 call sites) hasn't happened either. None of this can be done as a small build-verified increment the way 4.1–4.8 were — it touches every view file at once, and this environment has no way to visually exercise the SwiftUI app to catch behavioral regressions the compiler wouldn't. Flagging for explicit human sign-off before starting rather than attempting it as a natural continuation of 4.1–4.8's pattern.
...
**Current state (verified 2026-07-25): Phase 0–3 done in full (except 0.2/0.3's partial characterization coverage); Phase 4.1–4.8 done.** All 7 stores are extracted — `LocalizationStore`, `LogStore`, `ThaiHubStore`, `ProfilesStore`, `ModPacksStore`, `SavesStore`, `ModsStore`, `AppEnvironment` — every one with real Phase 3.1 protocol DI where the dependency exists yet, and every one with a forwarding shim on `StarHubTHViewModel` so **no view file has needed to change across all of Phase 4**. `StarHubTHViewModel` is ~770 lines, down from 2,102. `App/DependencyContainer.swift` exists but is still unused — nothing is registered in it or injected via `.environmentObject` at `MainView`. Test suite has grown to 182 assertions, including one dedicated suite per store using the Phase 3.2 stubs.

**Only 4.9 remains in Phase 4, and it is not started.** Deleting `StarHubTHViewModel` is a different shape of work than 4.1–4.8: those all specifically avoided touching any view file by keeping the ViewModel as a forwarding layer. Actually deleting it requires migrating every view (~20 files) from `@ObservedObject var vm: StarHubTHViewModel` to the specific `@EnvironmentObject` stores each one needs, wiring `DependencyContainer` into `MainView` for real, giving `alertMessage`/`showAlert` a home (view `@State` per the recipe, or a dedicated store), relocating the cross-store orchestrators (`refresh()`, `handleOpenURL`, `selectGameDir()`), and doing the `L(_:)` → `subscript(_:)` rename across ~408 call sites that Phase 4.1 explicitly deferred. None of this fits the small-build-verified-increment pattern the rest of Phase 4 used, and this environment has no way to visually exercise the SwiftUI app to catch behavioral regressions the compiler wouldn't flag. Get explicit sign-off before starting 4.9 rather than treating it as a natural continuation.

Lessons that applied across all of 4.1–4.8, for whoever picks up 4.9 or Phase 5:
1. **`@MainActor` timing.** The per-store recipe says `@MainActor final class`, but a store is only safe to annotate that way once its caller (the nonisolated `StarHubTHViewModel`, for as long as it exists) can call into it without a hard actor-isolation error. None of the 7 stores could use it yet — verify with a build, don't assume.
2. **Reactivity: check whether state mutates through an exposed setter or from inside the store's own methods.** If only the former (4.1's `currentLanguage`, 4.5's `importedModPack`), a manual `objectWillChange.send()` in the ViewModel's forwarding setter is enough. If the latter (true of most state in most stores — anything mutated from inside an async dispatch), subscribe with `store.objectWillChange.sink { ... }.store(in: &cancellables)` in `init()` — the `cancellables` Set on the ViewModel exists for exactly this and every store added it.
3. **Cross-store dependencies before every store exists.** A store needing data owned by a not-yet-extracted part of the ViewModel (`mods`, `gameDir`, `showModal`, `log`) takes it as a parameter or closure rather than holding a reference back into the ViewModel — see ThaiHubStore, ProfilesStore, ModPacksStore, ModsStore. One recurring subtlety: if a method needs to re-read that data *after* triggering a mutation elsewhere (ProfilesStore.applyProfileToFilesystem needs `mods` fresh after calling `scanMods()`), a snapshot value isn't enough — it needs a `() -> T` provider closure instead.
4. **Definite-initialization order.** A store whose `init()` needs another already-initialized property (not just a value type default) can't itself have a default value expression — it must be assigned inside `StarHubTHViewModel.init()`'s body. Swift's definite-initialization check then requires that assignment to happen before *any* closure captures `self`, even a closure that doesn't touch that specific property — order matters at the whole-`init()` level, not just per-statement.
5. **Testing a store whose method wraps `DispatchQueue.main.async` with no completion callback.** A test can't just assert immediately after calling it. Either the method has a real completion closure (assert inside that, after a `drainMainQueue()`-style semaphore wait queued after the call), or wrap the whole test suite's `run()` in the background-queue + `RunLoop.main` pump pattern already established in `Tests/main.swift` for `NXMParserTests`/`NexusCollectionTests`/`ModUpdateTests`/`ModPacksStoreTests`/`ModsStoreTests` — blocking that wait on the main thread deadlocks, since nothing would be left to pump the run loop.
