...
| `@MainActor` annotations | 10 of 10 (all 8 stores + `AlertStore` + `AppCoordinator`, plus `SmapiInstaller`/`DependencyContainer`) | every store |
| `DispatchQueue` calls | 8 — all legitimate background-queue work (Process wrapping, debug-log file write, live-tail dispatch source, 2 unrelated UI debounce timers) | ~0 |
| `@escaping` completion handlers | 20 — all synchronous callback params (`showModal`, `log`, `onInstalled`), not async completions | 0 |
...
- [x] 0.3 Characterization tests — this line was stale: `ModGraphTests.testPackModStatus` (5 cases: Nexus-ID match enabled/disabled, unique-ID fallback, nil Nexus ID, no match) already covered pack status, and `testEnableChain`/`testDisableChain`/`testChainingDisabled` already covered profile chain resolution for standalone mods. The one real gap — `enabledIDs`' group-handling branches (toggling a group, or a mod a group's child depends on) — had zero coverage; closed with `testEnableChainThroughGroup`/`testDisableChainCascadesIntoGroup`.
...
- [x] 3.1 Define protocols at each I/O boundary — all 9: `NexusAPIClient`, `ModScanning`, `ModInstalling`, `SaveStoring`, `SaveNoteStoring`, `ProfileStoring`, `SmapiInstalling`, `FilePicking`, `PreferenceStoring`.
- [x] 3.2 Rename implementations `Live*` (`NexusAPIService` → `LiveNexusAPIClient`), write `Stub*` for all 9 protocols in `Tests/Stubs/`
- [x] 3.3 Composition root (`App/DependencyContainer.swift`) — holds one instance of each dependency, defaults to `Live`. Not yet handed to anything; nothing to inject it into until Phase 4's first store.
- [x] 3.4 Confine AppKit (`FilePicking`) — `selectGameDir`, `openInstallModPanel`, `selectCustomAvatar` now go through `FilePicking`/`FilePicker` instead of constructing `NSOpenPanel` directly. `restoreModZip` and the `NSSavePanel` backup-export flows weren't named in this step and are untouched.
- [x] 3.5 Wrap `UserDefaults` (`PreferenceStoring`) — every `UserDefaults.standard` hit in `StarHubTHViewModel` now goes through `PreferenceStoring`, except `currentLanguage`'s property-initializer default (can't reach a sibling stored property before `self` exists) and the deprecated `.synchronize()` call (no protocol equivalent). Caching inside `PreferenceStore` itself is still open — not done.
- [x] 4.1 `LocalizationStore` — moved verbatim (currentLanguage, makeDateFormatter, localizedString/L/localizedTag) behind PreferenceStoring DI. StarHubTHViewModel forwards to it so the ~400 `vm.L(...)` call sites are untouched — that rename is separate, deliberately deferred work. Store is NOT @MainActor yet: the ViewModel calls it from a nonisolated context, and @MainActor here produced 5 hard actor-isolation errors, not warnings. Annotating stores @MainActor is 5.3's own pass, once every store exists.
- [x] 4.2 `LogStore` — logOutput/logEntries/isReadingSMAPILog/log()/loadSmapiLog()/smapi log watcher, moved verbatim. Unlike 4.1, this store mutates its own @Published state from inside its own methods (not just via an exposed setter), so the ViewModel now subscribes to `logStore.objectWillChange` and re-sends it — see the new `cancellables` Set on the ViewModel, reusable by later stores. Same @MainActor deferral as 4.1.
- [x] 4.3 `ThaiHubStore` — thaiTranslations/viewingThaiMod/fetch/evaluate/install, moved verbatim. Needs gameDir (AppEnvironment, 4.8) and mods (ModsStore, 4.7), neither extracted yet — the affected methods take those as parameters instead of the store holding a back-reference; only the ViewModel's 3 forwarding methods will need to change once 4.7/4.8 land. Sharper init-order lesson found here: definite-initialization blocks ANY self-capturing escaping closure until every no-default stored property is assigned, not just the ones that closure reads — a store needing a same-object dependency (thaiHubStore needs localizationStore) must be assigned in init() before any earlier store's objectWillChange subscription captures self.
...
- [x] 4.8 `AppEnvironment` — gameDir/steamUsername/steamAvatarPath/smapiInstalledVersion/showSmapiAlerts/smapiInstaller, real DI with PreferenceStoring + LocalizationStore. gameDir's didSet split: AppEnvironment persists, ViewModel's forwarding setter calls refresh() (cross-store). One documented behavior simplification — startup no longer runs refresh() twice redundantly, just once. ViewModel is ~770 lines, down from 2,102.
- [x] 4.9 Delete `StarHubTHViewModel` — done. `StarHubTHViewModel.swift` no longer exists. Two new pieces absorbed what had no store: `App/AlertStore.swift` (replaces `alertMessage`/`showAlert`/`showModal`) and `App/AppCoordinator.swift` (the ~40 cross-store orchestration methods — `refresh()`, `handleOpenURL()`, `selectGameDir()`, `toggleMod()`, the install/backup/download family — as near-verbatim relocations, holding plain references to all 8 stores + `AlertStore`, owning zero `@Published` state itself). `requestedTab` was replaced by reusing the existing `.jumpToMod`-style `NotificationCenter` pattern (new `.switchToTab`, in `Support/Notification+SwitchToTab.swift`). `chainToggleDependencies`/`nexusApiKey` moved onto `AppEnvironment`. `MainView` is now the real composition root — its `init()` constructs all 8 stores + `AlertStore` + `AppCoordinator`, sourcing services from `DependencyContainer` (finally used) instead of `.shared`/`Live*` directly, and exposes them via `.environmentObject`. Every view now holds only the specific stores/coordinator it needs. Also folded in fixes for the 4 view→singleton bypasses an earlier audit found (`ModDetailView`/`ModListView` called `LiveNexusAPIClient.shared` directly for Nexus info/endorse; `ModProfilesView` called `ProfileManager.shared`/`CollectionInstaller.shared`; `SavesView` called `SaveNotesStore.shared`) — each now routes through its store, with 2 new `ModsStore` methods and 2 new `ProfilesStore` methods added along the way. The `L(_:)` → `subscript(_:)` rename (Phase 4.1's own deferred item, ~435 call sites by the time 4.9 landed) was deliberately kept out of scope — every `vm.L(...)` became `<store>.L(...)`, same method-call syntax, to keep each diff mechanically reviewable. Sequenced as 10 per-view-tree commits (each build+test verified, exactly as low-risk as 4.1–4.8) between an additive setup commit and the final MainView+deletion commits — turned out **not** to require one big-bang commit after all; see lessons below.
- [x] 5.1 `NexusAPIClient` → `async throws` — `LiveNexusAPIClient`'s 3 private helpers + 5 protocol methods, plus `NexusDownloader.downloadUpdate`. Bundled with 5.2 since the signature change breaks every caller immediately.
- [x] 5.2 Convert remaining `@escaping` completion handlers — done cluster by cluster, one commit each: Mods/Nexus (`syncTagFromNexus`/`fetchNexusInfo`/`endorseMod`/`syncAllTagsFromNexus`/`downloadAndInstallUpdate`), ModPacks/download, install (`ModInstaller` + `ModsStore.installMod` family), backup/restore (`ModsStore.backupMod`/`restoreModZip`), ThaiHub (`installThaiTranslation`), Profiles (see deviation below), SMAPI installer (`SmapiInstaller` — the largest cluster). Every SwiftUI call site wraps the now-`async` call in `Task { }`.
- [x] 5.3 Annotate every store `@MainActor` — all 8 stores + `AlertStore` + `AppCoordinator`, plus `SmapiInstaller` and `DependencyContainer` (both in the same shape, not on the original list but needed the same fix). Sequenced *last* among 5.1–5.6 deliberately — see lesson 4 below.
- [x] 5.4 Filesystem services → `Sendable`, not `actor` — deviation, see lesson 1.
- [x] 5.5 Conform models to `Sendable` — all 14 types in `Models/`, mechanical.
...
- [x] 7.1 Typed error enum per service — new `SaveStorageError` (SaveManager, 8 cases), `ProfileApplyError` (ProfileManager, carries failed mod names). `ModInstallerError`/`NexusDownloaderError` already existed from earlier phases.
- [x] 7.2/7.3 Audit `try?`/bare `catch {}` — every one in `SaveManager`, `ProfileManager`, `ModInstaller`, `SmapiInstaller` now either propagates through a typed throw or has an inline comment explaining why it's genuine best-effort cleanup/rollback. **Not exhaustive across the whole codebase** — see lesson 9. The ~170 remaining `try?` sites elsewhere are overwhelmingly `-> Optional` parse-and-return-nil patterns (SaveFileParser, ModManifestParser, ModDetailView's readme loader, JSON/regex construction from hardcoded literals) where the optional return type already self-documents "absence is expected."
- [x] 7.4 Replace `-> Bool` status returns with `throws` — `SaveStoring`'s 8 methods, `ProfileStoring.applyProfileToFilesystem`, `ModsStore`'s install family (`installMod`/`installModFromZip`/`installModFromFolder`/`handleInstallResult`). `ModsStore.syncTagFromNexus`'s discarded-everywhere Bool dropped to plain `async`. `ModPacksStore`'s download family (`downloadModFromNexus`/`startDownload`/`downloadAllMissing`) deliberately kept its Bool-based, continuation-bridged design — see lesson 10.
- [x] 7.5 Route `print(` calls into `LogStore` — all 18 removed: routed into an already-injected `LogStore`/`log` closure (3), into `alertStore` where the failure had zero prior user feedback (1), or replaced with a `try?` + comment where the call site is a stateless parser with no reachable log channel and no user-facing impact (1, `ModManifestParser`).
- [x] 7.6 Surface typed errors in UI + localization keys — `SaveStorageError`/`ProfileApplyError` reuse existing generic `vm_*_error` keys with the underlying error's own description appended as detail (matching `SmapiInstallOutcome`'s existing messageKey+detail shape), so no new keys were needed there. One new key pair added (`vm_sync_tag_failed`, both `assets/en.json`/`assets/th.json`) for `syncTagFromNexus`, whose failure path had no messaging at all before.
- [x] 8 View decomposition — all 7: `ModListView` (1183→207, +7 files), `SavesView` (809→197, +7), `MainView` (723→273, +4), `ModConfigEditorView` (528→390, +4), `ModProfilesView` (442→129, +2), `ModDetailView` (389→237, +3), `ModPacksView` (402→92, +2). Every extraction was an already-separate `View` struct just sharing a file — a pure move, same class of change as Phase 1.2 — except `ModConfigEditorView`'s `ConfigFieldRow`/`ConfigSectionTree` (private free functions promoted to real View structs) and `MainView`'s new `MainSidebarView` (its sidebar was inline in `body`, not a separate struct). No `#Preview`s added — not part of this codebase's existing convention, and out of scope for a pure-move pass.
- [x] 9.1 `scripts/check_standards.py` — 6 whole-codebase checks (get-prefix, non-final class, `.shared` outside `App/`, uncommented `DispatchQueue`, files >400 lines, `@Published` without `private(set)`). Warnings-only by default (exit 0); `--strict`/`STARHUB_STANDARDS_STRICT=1` promotes findings to a build failure — not flipped on yet, see note below.
- [x] 9.2 Wire into `build_app.py` (runs first, advisory, never fails the build unless the strict env var is set) + `.github/workflows/build.yml` (macOS runner, runs the check then `build_app.py`/`run_tests.py` on every PR/push to `main`)
- [ ] 9.3 SwiftLint / swift-format config (optional) — skipped; explicitly optional in the plan and the custom script already covers the repo-specific rules no generic linter knows
- [x] 9.4 Update `CHANGELOG.md` — added an `[Unreleased]` entry summarizing the refactor for contributors, no version bump (no user-facing behavior change)

**Current state (verified 2026-07-26): Phases 0–9 all done** (9.3 skipped, explicitly optional) (0.2 still open — a baseline build log is only useful captured before refactoring starts, and would be moot to backfill 7 phases in; 7.2/7.3's audit is deep-but-not-exhaustive — see lesson 9). All 8 stores + `AlertStore` + `AppCoordinator` + `SmapiInstaller` + `DependencyContainer` are `@MainActor`. Every completion-handler-based I/O path is `async`/`await`; every model and the flagged `.shared` singletons are `Sendable`. `build_app.py` compiles the whole app under full Swift 6 language mode with zero warnings. Test suite: 198 assertions. Phase 6's naming sweep is done: `get`-prefixes gone, argument labels read as phrases, `ModItem`→`Mod`/`LogEntry`→`LogLine`, `modTag`→`tag`, `fm`→`fileManager`. `6.1`'s `vm` sweep was already a byproduct of 4.9. `uniqueId`→`id` was deliberately skipped (lesson 8) — that swap would violate §2.5's own group-identity warning, not honor it. Phase 7's error handling is done: `SaveStorageError`/`ProfileApplyError` typed errors replace 10 `-> Bool` methods across `SaveStoring`/`ProfileStoring`, `ModsStore`'s install family throws `ModInstallerError` directly instead of discarding it into an already-unused Bool, every `print(` is gone, and two previously-100%-silent failure paths (backup/delete-backup buttons, Nexus tag sync) now show the user something. Phase 8's view decomposition is done: all 7 oversized views split into 29 new files, verified build+test green after each and the running app screenshotted mid-pass to confirm the sidebar split rendered identically.
...
2. **(New in 5.6) Don't assume "the Timer/FileHandle properties are there so the feature must be live" — check.** `LogStore`'s SMAPI tailing looked like a live-polling feature from its stored properties alone; grepping for `Timer.scheduledTimer` found zero call sites — it was a one-shot read wearing a live-tailer's clothes. Worth verifying against actual behavior, not just the type signatures, before deciding a conversion is "just" a modernization.
3. **(New in 5.3) Sequence `@MainActor` last, after the async conversions, not first.** Annotating a store `@MainActor` before its methods are `async` just forces every synchronous caller onto an awkward `Task { @MainActor in }` wrapper for no benefit. Doing the async conversions first means `@MainActor` mostly *deletes* code (redundant `DispatchQueue.main.async` hops) instead of adding it.
4. **(New in 5.3) A type's default-argument *values* are evaluated in a nonisolated context, regardless of the enclosing function's own actor isolation.** Marking `DependencyContainer` (or its init) `@MainActor` does not let `= SmapiInstaller()`/`= FilePicker()` live as default parameter values once those types are themselves `@MainActor` — Swift errors on "main actor-isolated initializer in a nonisolated context" at the default-value expression itself. Fix: default the parameter to `nil` and construct the real value inside the init's body, which *is* isolated correctly since the whole type is.
5. **(New in 5.3) A `@MainActor` type's conformance to a plain (non-isolated) protocol only stops warning once *every* conformer and *every* caller of that protocol are on the same actor.** Hit with both `SaveNotesStore: SaveNoteStoring` and `FilePicker: FilePicking` — marking just the conforming type (or just the extension) `@MainActor` left a residual `#ConformanceIsolation` warning; it only cleared once the *protocol itself* was also marked `@MainActor` (safe once every real caller — the stores — was `@MainActor` too, which by 5.3 they all were). Similarly, a protocol requiring `Sendable` (`NexusAPIClient`, `ModInstalling`) needs its `Stub*` test doubles marked `@unchecked Sendable` if they hold mutable `var` canned results, and a struct's `Sendable` conformance declared via an `extension` in a *different file* than the struct itself triggers a "must occur in the same source file" error — declare it directly on the type instead.
6. **(New in 5.2) Converting a synchronous test function to `async` can turn a previously-harmless `DispatchSemaphore.wait(timeout:)` into an "unavailable from asynchronous contexts" warning.** Hit in `NXMParserTests`/`ModUpdateTests`' still-unconverted raw `URLSession.downloadTask` steps — harmless (the semaphore blocks a `Task.detached` carrier thread, not the actor), but worth knowing before converting a test's outer function to `async` for unrelated reasons.
7. **(Unchanged from 4.9) Cross-store dependencies, definite-initialization order, and the `Task.detached` + `RunLoop.main` pump pattern for tests with a `DispatchQueue.main` dependency** all still apply — see 4.9's own lessons above. One refinement: mixing plain top-level `await` with that manual-pump pattern in the *same file* destabilizes the pumped tests unpredictably (hit converting `Tests/main.swift`) — route every async suite through one consistent thread model (a `runSuite` helper wrapping `Task.detached` + the pump), even the ones that don't themselves need the pump.
8. **(New in 6.5) A stale plan line can contradict the standards doc it's supposed to implement — check both before renaming.** 6.5's own table row says `uniqueId` → `id`, but `Mod.id` already returns `folderName` (correct, per §2.5: stable across enable/disable) and `Mod`'s raw `uniqueId` field is still constructed empty for group rows in `ModScanner` (`uniqueId: ""`) — exactly the collision SWIFT_STANDARDS.md §2.5 explicitly warns not to route `id` through: "Do not 'fix' this by switching `id` to `uniqueId` — that would collide every group onto one identity and is strictly worse." The 6.5 line predates the Phase 2.4 `Mod.Kind` fix and was never updated. Did the safe half of 6.5 (`modTag` → `tag`) and left `uniqueId`/`id` as they are.
9. **(New in 7.2/7.3) An exhaustive line-by-line `try?`/`catch {}` audit of the whole codebase is a different-sized task than fixing the ones the standards doc calls out by name.** Went deep on the four services with the highest concentration and the most ambiguous failure modes (`SaveManager`, `ProfileManager`, `ModInstaller`, `SmapiInstaller` — 38 of the ~118 occurrences at the time), since those are exactly the ones §7.1's own worked example ("a permissions error, a missing directory, and an empty saves folder are all reported as 'you have no saves'") is about. Left the remaining ~170 across the rest of the codebase for a future pass — nearly all of them are `-> Optional` parse-and-return-nil patterns (file/JSON/regex parsing) whose signature already tells the reader "absence is legitimate," a materially different risk profile than a `Bool`-returning mutation swallowing *why* it failed.
10. **(New in 7.4) Not every `-> Bool` is the anti-pattern the rule is about.** `ModPacksStore.downloadModFromNexus`/`startDownload`/`downloadAllMissing` return `Bool` through a `(URL, @escaping (Bool) -> Void) -> Void` completion callback set up in Phase 5.2 — but every failure branch inside them already calls `logStore.log(...)` with a specific, distinguishable reason before returning `false`. The rule exists to stop a caller from losing information ("you have no saves" hiding three different causes); here no information is lost, it's just carried by a side-channel log instead of the return value. Converting this one anyway to satisfy the letter of "no `-> Bool`" would mean unwinding a deliberate Phase 5 design (the continuation-bridged completion callback) for no behavioral gain — left as-is, documented rather than silently skipped.
11. **(New in 0.3) A tracked gap can already be closed without anyone updating the tracker.** 0.3 said pack status and profile chain resolution were untested; `ModGraphTests` had actually covered both thoroughly (`testPackModStatus`'s 5 cases; `testEnableChain`/`testDisableChain`/`testChainingDisabled`) by the time this was checked — just never reflected here. The one genuine hole, found by reading the code against the tests rather than trusting the checkbox, was `enabledIDs`' group-handling branches: every existing chain test toggles a standalone mod, none exercise toggling a group or a mod a group's child depends on.
12. **(New in 8) Check whether a "decomposition" is already done before assuming it's a rewrite.** All 7 files the plan named turned out to already contain their target subviews as separate `View` structs — nobody had ever moved them to their own files, but nobody had merged them into one giant `body` either. Reading each file fully before touching it turned what looked like 7 risky view-splitting exercises into 29 mechanical file moves (the same "if the build breaks, it wasn't a pure move" guarantee as Phase 1.2). The two genuine exceptions — `ModConfigEditorView`'s `renderItemRow`/`renderNodeChildren` (private free functions, not View structs) and `MainView`'s inline sidebar `body` block — needed real conversion work, not just a move, and got it: `ConfigFieldRow`/`ConfigSectionTree` as real structs (dropping an `AnyView` type-erasure the free-function version needed to recurse), and a new `MainSidebarView`.
13. **(New in 9.1) A whole-codebase static check is not the same thing as "the codebase is clean."** Running `check_standards.py` on a repo the plan calls "done" still reports 57 findings: 44 `@Published` without `private(set)` (pervasive by design in several stores — `SavesStore`/`ModsStore`/`ThaiHubStore` filter/selection state is meant to be set directly by views, not just read), 7 `DispatchQueue` sites (all previously audited as legitimate — Process wrapping, live-tail dispatch source, 2 UI debounce timers — but the checker can't read the prose justification in this doc's own metrics table, only an inline comment), 4 files over 400 lines (`ModsStore`, `SaveManager`, `LiveNexusAPIClient`, `L10n` — none are views, so Phase 8's "views > 400 lines: 0" metric was never contradicted, but 9.1's own wording ("files over 400 lines") is broader than that), and 2 real leftover `.shared` call sites outside `App/` (`ProfilesStore`'s `CollectionInstaller.shared`, `NexusDownloader`'s default-parameter `LiveNexusAPIClient.shared`) that Phase 3.3 never fully closed. None of these were fixed as part of 9.1/9.2 — building the checker and fixing 57 pre-existing, individually-judgment-call findings are different-sized tasks, and the plan's own text ("start as warnings, promote to errors") anticipates exactly this: the script ships non-blocking, cleanup is separate later work.

Note: this doc was edited concurrently by multiple sessions working the same plan — always re-verify against `git log` before trusting these checkboxes blindly.
