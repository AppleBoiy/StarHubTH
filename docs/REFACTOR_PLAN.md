...
**Current state (verified 2026-07-25): Phase 0–2 done except 0.2/0.3 (partial characterization coverage); Phase 3 done in full.** Tree already has the `App/Features/DesignSystem/Localization/Support` layout. All 9 I/O-boundary protocols exist, `NexusAPIService` is renamed `LiveNexusAPIClient`, every protocol has a `Stub*` in `Tests/Stubs/`, `App/DependencyContainer.swift` exists as the composition root (not yet handed to anything), and `StarHubTHViewModel` now goes through `FilePicking`/`PreferenceStoring` instead of `NSOpenPanel`/`UserDefaults.standard` at the call sites named in 3.4/3.5 specifically — other panel/UserDefaults call sites elsewhere in the codebase are untouched by design. `.shared` singletons are still constructed directly everywhere (SaveManager.shared, ProfileManager.shared, etc.) — DependencyContainer isn't consumed by anything yet. Next open step is **Phase 4** (split the god ViewModel into per-feature stores) — start with 4.1 `LocalizationStore`, the plan's own recommended first store since it's "nearly standalone" and every view touches it. Note: this doc was edited concurrently by multiple sessions working the same plan — always re-verify against `git log` before trusting these checkboxes blindly.
