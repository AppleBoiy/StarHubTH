...
**Current state (verified 2026-07-25): Phase 0–2 done except 0.2/0.3 (partial characterization coverage); Phase 3 done in full; Phase 4.1–4.2 done.** Tree already has the `App/Features/DesignSystem/Localization/Support` layout. All 9 I/O-boundary protocols exist, `NexusAPIService` is renamed `LiveNexusAPIClient`, every protocol has a `Stub*` in `Tests/Stubs/`, `App/DependencyContainer.swift` exists as the composition root (not yet handed to anything), and `StarHubTHViewModel` goes through `FilePicking`/`PreferenceStoring` at the 3.4/3.5-named call sites. Two stores extracted so far — `LocalizationStore` and `LogStore` — both with the ViewModel forwarding to them rather than views being migrated to `@EnvironmentObject` yet (recipe steps 4–5 are deferred; nothing is registered in `DependencyContainer` or injected via `.environmentObject` at `MainView`). `.shared` singletons are still constructed directly everywhere else. Next open step is **4.3 `ThaiHubStore`**.

Two general lessons for whoever does 4.3 onward:
1. **`@MainActor` timing.** The per-store recipe says `@MainActor final class`, but a store is only safe to annotate that way once its caller (still the nonisolated `StarHubTHViewModel`) can call into it without a hard actor-isolation error. Neither 4.1 nor 4.2 could use it — verify with a build, don't assume.
2. **Reactivity: check whether state mutates through an exposed setter or from inside the store's own methods.** If only the former (4.1's `currentLanguage`), a manual `objectWillChange.send()` in the ViewModel's forwarding setter is enough. If the latter (4.2's `LogStore`, which appends to `logEntries` from inside `log()` and `loadSmapiLog()`, often via a background-queue dispatch), you need `store.objectWillChange.sink { ... }.store(in: &cancellables)` in `init()` — the `cancellables` Set already exists on the ViewModel for this.
