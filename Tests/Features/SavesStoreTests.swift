...
@MainActor
final class SavesStoreTests: XCTestCase {
    private func save(_ folderName: String, playerName: String = "Player") -> SaveGameInfo {
        SaveGameInfo(
    ...
    private func makeStore(saves: [SaveGameInfo] = []) -> (SavesStore, StubSaveStoring, StubSaveNoteStoring, StubFilePicking) {
    ...
    func testReloadSaves() {
        let (store, _, _, _) = makeStore(saves: [save("Farm1"), save("Farm2")])
        store.reloadSaves()
        XCTAssertEqual(store.saves.count, 2, "reloadSaves reads saves from the backing store")
    }

    func testEditSaveSuccessAndFailure() {
        ...
        XCTAssertEqual(messages.count, 1, "a successful edit shows exactly one modal")

        stub.operationsSucceed = false
        store.editSave(info: save("Farm1"), newName: "New", newFarm: "F", newFav: "", newMoney: 0, newTotalMoneyEarned: 0, newMaxHealth: 100, newMaxStamina: 100, newGoldenWalnuts: 0, newQiGems: 0, newClubCoins: 0, newSpouse: "", showModal: showModal)
        XCTAssertEqual(messages.count, 2, "a failed edit also shows a modal")
    }
    ...
    func testDeleteSave() {
        ...
        XCTAssertTrue(succeeded, "deleteSave shows a modal on success")
    }

    func testSavesHierarchyBuildsParentChildTree() {
        let (store, _, _, _) = makeStore(saves: [save("Farm1"), save("Farm1_branch")])
        store.reloadSaves()

        let roots = store.savesHierarchy
        XCTAssertEqual(roots.count, 1, "a branch save nests under its parent instead of appearing at the root")
        XCTAssertEqual(roots.first?.children.count, 1, "the parent has exactly one child")
    }

    func testSavesHierarchyFiltersByTag() {
        ...
        let roots = store.savesHierarchy
        XCTAssertEqual(roots.count, 1, "the tag filter keeps only saves carrying that tag")
        XCTAssertEqual(roots.first?.info.folderName, "Farm1", "the surviving save is the one with the matching tag")
    }

    func testAvailableFilterTags() {
        let (store, _, noteStoring, _) = makeStore(saves: [save("Farm1"), save("Farm2")])
        store.reloadSaves()
        noteStoring.setNote("", tag: "star", forSave: "Farm1", customIconPath: nil)

        XCTAssertEqual(store.availableFilterTags, ["star"], "availableFilterTags collects distinct non-empty tags across saves")
    }

    func testSetAvatarAndGetNote() {
        let (store, _, _, _) = makeStore()
        store.setAvatar(forSave: "Farm1", iconPath: "/tmp/icon.png")
        XCTAssertEqual(store.note(for: "Farm1").customIconPath, "/tmp/icon.png", "setAvatar persists the icon path via SaveNoteStoring")
    }

    func testSelectCustomAvatarCopiesPickedFile() {
        ...
        XCTAssertTrue(completedPath != nil, "selectCustomAvatar's completion fires with the copied file's path")
        XCTAssertEqual(store.note(for: "Farm1").customIconPath, completedPath, "the copied path is persisted as the save's avatar")
    }

    func testEditingSaveLoadsInventory() {
        ...
        XCTAssertEqual(store.inventoryToEdit.count, 1, "setting editingSave loads that save's inventory")

        store.editingSave = nil
        XCTAssertTrue(store.inventoryToEdit.isEmpty, "clearing editingSave clears the loaded inventory")
    }
}
